...
export const metadata = {
  ...
  description: "Import types from other GitHub repositories to share common data structures across projects.",
}
...
export default function DependenciesPage() {
  return (
    <Prose>
      ...
      <p>
        To add a dependency, open your <code>skir.yml</code> file and add an entry to the <code>dependencies</code> section. The key is the GitHub repository identifier (<code>owner/repo</code> prefixed with <code>@</code>), and the value is the Git tag or release version you want to use.
      </p>
  ...
  "@my-org/user-service-skir": v3.5.0`}</CodeBlock>
      <p>
        When you run <code>npx skir gen</code>, Skir will automatically fetch these dependencies and cache them in the <code>skir-external/</code> directory.
      </p>
      <Note type="info">
        <p>The <code>skir-external/</code> directory should be added to your <code>.gitignore</code>.</p>
      </Note>
...
struct QuestCollection {
  ...
  quests: [Quest|quest_id];
      ...
      <p>
        For languages which allow <code>@</code> symbols in directory names (like JavaScript), the code generated from external dependencies is placed in <code>{'skirout/@{owner}/{repo}'}</code>:
      </p>
...
import { Quest } from "../skirout/@gepheum/fantasy-game-skir-example/fantasy_game.js"`}</CodeBlock>
      <p>
        For languages which require every directory name to be a valid identifier (like Python), the generated code is placed in <code>{'skirout/external/{owner}/{repo}'}</code>, with dashes replaced by underscores:
      </p>
...
from skirout.external.gepheum.fantasy_game_skir_example import fantasy_game_skir`}</CodeBlock>
      ...
      <p>
        Go to your GitHub settings and <a href="https://github.com/settings/tokens" target="_blank" rel="noopener noreferrer">generate a new Personal Access Token (Classic)</a> with the <code>repo</code> scope (for full control of private repositories) or just <code>read:packages</code> if applicable, though usually <code>repo</code> is needed for private source code access.
      </p>
      ...
      <p>Tell Skir which environment variable to look for in your <code>skir.yml</code>:</p>
      <CodeBlock language="yaml" filename="skir.yml">{`# skir.yml
githubTokenEnvVar: MY_GITHUB_TOKEN`}</CodeBlock>
      <p>Skir will now read the token from <code>MY_GITHUB_TOKEN</code> to authenticate requests.</p>

      <h3>4. Set up GitHub Actions CI</h3>
      <p>
        When running Skir in a Continuous Integration (CI) environment like GitHub Actions, you can use the built-in <code>{'${{ secrets.GITHUB_TOKEN }}'}</code> to access other repositories within the same organization, or use a repository secret if you need broader access.
      </p>
  ...
  - name: Install dependencies and generate code
    ...
    env:
      ...
      <Note type="warning">
        <p>
          If the dependency is in a different private repository that the default <code>GITHUB_TOKEN</code> cannot access, you will need to create a Personal Access Token (PAT), store it as a Repository Secret (e.g., <code>PAT_TOKEN</code>), and use that instead.
        </p>
      </Note>
    </Prose>
  )
